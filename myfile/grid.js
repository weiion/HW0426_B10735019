...
var mapSketch = function(p5j){
    ...
    var quake = function (c1,c0,mag){
      ...
      this.marker = L.circle([this.c1, this.c0], { // 緯度在前面
                ...
                fillOpacity: 0.3,
                stroke: false, // 取消邊線
                radius: this.mag*20000 // 強度 乘上大小單位為公尺
              });
      ...
      this.update =function(){
        ...
        if (p5j.map.getBounds().contains(ll)){
            ...
            if (this.layerGroup.open){ // 若本layerGroup 打開
                ...
                //p5j.ellipse(pix.x,pix.y,this.ani,this.ani);//pix.x,pix.y為圓心位置 , this.ani為圓半徑
                p5j.fill(50);
                p5j.rect(0, 0, 1200, 600);
                p5j.erase();
                p5j.ellipse(p5j.mouseX-30,p5j.mouseY-30,200);
                p5j.noErase();
...
/*                if (this.inverse){
    ...
    p5j.setup = function(){
        // 加入一個畫布
    	p5j.createCanvas(1200,600).style('z-index:400'); // ****一層z-index=100*****//可進行動畫ㄉ新畫布
